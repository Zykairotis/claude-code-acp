...
```bash
claude-flow github swarm [options]
```
...
```bash
claude-flow github swarm --repository owner/repo
```

### Maintenance-focused swarm

```bash
claude-flow github swarm -r owner/repo -f maintenance --issue-labels
```

### Development swarm with PR automation

```bash
claude-flow github swarm -r owner/repo -f development --auto-pr --code-review
```

### Full-featured triage swarm

```bash
claude-flow github swarm -r owner/repo -a 8 -f triage --issue-labels --auto-pr
```
