...
```bash
claude-flow auto agent [options]
```
...
```bash
claude-flow auto agent --task "Build a REST API with authentication"
```

### Constrained spawning

```bash
claude-flow auto agent -t "Debug performance issue" --max-agents 3
```

### Analysis only

```bash
claude-flow auto agent -t "Refactor codebase" --no-spawn
```

### Minimal strategy

```bash
claude-flow auto agent -t "Fix bug in login" -s minimal
```
