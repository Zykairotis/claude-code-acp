...
claude-flow agent spawn load-balancer --type coordinator

# Start load balancing
claude-flow load-balance --swarm-id <id> --strategy adaptive

# Monitor load distribution
claude-flow agent-metrics --type load-balancer

# Adjust balancing parameters
claude-flow config-manage --action update --config '{"stealThreshold": 5, "agingBoost": 10}'
```
...
claude-flow metrics-collect --components ["load-balancer", "task-queue"]
```
