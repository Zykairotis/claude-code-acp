...
claude-flow health-check --components ["swarm", "agents", "coordination"]

# Collect specific metrics
claude-flow metrics-collect --components ["cpu", "memory", "network"]

# Monitor SLA compliance
claude-flow sla-monitor --service swarm-coordination --threshold 99.9
```

### Alert Configuration
```bash
# Configure performance alerts
claude-flow alert-config --metric cpu_usage --threshold 80 --severity warning

# Set up anomaly detection
claude-flow anomaly-setup --models ["statistical", "ml", "time_series"]

# Configure notification channels
claude-flow notification-config --channels ["slack", "email", "webhook"]
```
