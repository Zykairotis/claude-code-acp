...
claude-flow github release-create \
  --version "2.0.0" \
  --build-artifacts \
...
claude-flow sparc pipeline "Release v2.0.0 with full validation"
```
...
claude-flow github version-suggest \
  --current v1.2.3 \
  --analyze-commits \
  --check-compatibility \
  --suggest-pre-release
...
claude-flow github release-test \
...
claude-flow github release-deploy \
...
claude-flow github multi-release \
...
claude-flow github performance-test \
  --baseline v1.9.0 \
  --candidate v2.0.0 \
...
claude-flow github rollback-config \
  --triggers '{
    "error-rate": ">5%",
    "latency-p99": ">1000ms",
    "availability": "<99.9%",
...
claude-flow github release-compliance \
