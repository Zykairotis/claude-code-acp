...
claude-flow hive-mind init --config hive-config.json
```
...
claude-flow hive-mind spawn "Research AI patterns" --queen-type strategic

# Tactical queen with max workers
claude-flow hive-mind spawn "Implement API" --queen-type tactical --max-workers 12

# Adaptive queen with consensus
claude-flow hive-mind spawn "Optimize system" --queen-type adaptive --consensus byzantine

# Generate Claude Code commands
claude-flow hive-mind spawn "Build full-stack app" --claude
```
...
# Pause a session
claude-flow hive-mind pause <session-id>
...
claude-flow sparc tdd "User authentication" --hive-mind
...
claude-flow hive-mind spawn "Analyze repo quality" --objective "owner/repo"

# PR review coordination
claude-flow hive-mind spawn "Review PR #123" --queen-type tactical
```
...
```bash
claude-flow hive-mind spawn "Research ML frameworks" --queen-type strategic
```

**Tactical Queens** - For implementation and execution
```bash
claude-flow hive-mind spawn "Build authentication" --queen-type tactical
```

**Adaptive Queens** - For optimization and dynamic tasks
```bash
claude-flow hive-mind spawn "Optimize performance" --queen-type adaptive
```
...
claude-flow hive-mind memory --gc

# Optimize database
claude-flow hive-mind memory --optimize

# Export and clear
claude-flow hive-mind memory --export --clear
```
...
claude-flow hive-mind spawn "..." --consensus weighted

# Or use simple majority
claude-flow hive-mind spawn "..." --consensus majority
```
...
```bash
# Frontend hive
claude-flow hive-mind spawn "Build UI" --name frontend-hive

# Backend hive
claude-flow hive-mind spawn "Build API" --name backend-hive
...
claude-flow hive-mind export <session-id> --output backup.json

# Import session
claude-flow hive-mind import backup.json
```
...
```bash
# Spawn research hive
claude-flow hive-mind spawn "Research GraphQL vs REST" \
  --queen-type adaptive \
  --consensus byzantine
...
```bash
# Review coordination
claude-flow hive-mind spawn "Review PR #456" \
  --queen-type tactical \
  --max-workers 6
