...
```bash
claude-flow bottleneck detect
```

### Generate Performance Report
```bash
claude-flow analysis performance-report --format html --include-metrics
```

### Analyze and Auto-Fix
```bash
claude-flow bottleneck detect --fix --threshold 15
```
...
#### Command Syntax
```bash
claude-flow bottleneck detect [options]
```
...
claude-flow bottleneck detect --swarm-id swarm-123 -t 24h

# Export detailed analysis
claude-flow bottleneck detect -t 24h -e bottlenecks.json

# Auto-fix detected issues
claude-flow bottleneck detect --fix --threshold 15

# Low threshold for sensitive detection
claude-flow bottleneck detect --threshold 10 --export critical-issues.json
```
...
#### Command Syntax
```bash
claude-flow analysis performance-report [options]
```
...
claude-flow analysis performance-report --compare swarm-123 --format markdown
...
claude-flow swarm monitor --interval 5
...
async function analyzePerformance() {
  ...
  const bottlenecks = await runCommand(
    'claude-flow bottleneck detect --format json'
  );
...
claude-flow bottleneck detect --threshold 10

# Check cache performance
claude-flow cache manage --action stats
...
claude-flow bottleneck detect --time-range 1h
...
claude-flow analysis performance-report --sections metrics

# Review cache strategy
claude-flow cache manage --action analyze

# Enable cache warming
claude-flow bottleneck detect --fix
```
...
- `claude-flow swarm monitor` - Real-time monitoring
- `claude-flow token usage` - Token optimization analysis
- `claude-flow cache manage` - Cache optimization
- `claude-flow agent metrics` - Agent performance metrics
- `claude-flow task status` - Task execution analysis
