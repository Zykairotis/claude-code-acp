...
claude-flow hook pre-task --description "Implement authentication"

# Post-edit hook (auto-formats and stores in memory)
claude-flow hook post-edit --file "src/auth.js" --memory-key "auth/login"

# Session end hook (saves state and metrics)
claude-flow hook session-end --session-id "dev-session" --export-metrics
```
...
```bash
claude-flow hook pre-edit [options]
...
Examples:
  claude-flow hook pre-edit --file "src/auth/login.js"
  claude-flow hook pre-edit -f "config/db.js" --validate-syntax
  claude-flow hook pre-edit -f "production.env" --backup-file --check-conflicts
```
...
```bash
claude-flow hook pre-bash --command <cmd>
...
Examples:
  claude-flow hook pre-bash -c "rm -rf /tmp/cache"
  claude-flow hook pre-bash --command "docker build ." --estimate-resources
```
...
```bash
claude-flow hook pre-task [options]
...
Examples:
  claude-flow hook pre-task --description "Implement user authentication"
  claude-flow hook pre-task -d "Continue API dev" --load-memory
  claude-flow hook pre-task -d "Refactor codebase" --optimize-topology
```
...
```bash
claude-flow hook pre-search --query <query>
...
Examples:
  claude-flow hook pre-search -q "authentication middleware"
```
...
```bash
claude-flow hook post-edit [options]
...
Examples:
  claude-flow hook post-edit --file "src/components/Button.jsx"
  claude-flow hook post-edit -f "api/auth.js" --memory-key "auth/login"
  claude-flow hook post-edit -f "utils/helpers.ts" --train-patterns
```
...
```bash
claude-flow hook post-bash --command <cmd>
...
Examples:
  claude-flow hook post-bash -c "npm test" --update-metrics
```
...
```bash
claude-flow hook post-task [options]
...
Examples:
  claude-flow hook post-task --task-id "auth-implementation"
  claude-flow hook post-task -t "api-refactor" --analyze-performance
  claude-flow hook post-task -t "bug-fix-123" --store-decisions
```
...
```bash
claude-flow hook post-search --query <query> --results <path>
...
Examples:
  claude-flow hook post-search -q "auth" -r "results.json" --train-patterns
```
...
```bash
claude-flow hook mcp-initialized --swarm-id <id>
...
```bash
claude-flow hook agent-spawned --agent-id <id> --type <type>
...
```bash
claude-flow hook task-orchestrated --task-id <id>
...
```bash
claude-flow hook neural-trained --pattern <name>
...
```bash
claude-flow hook memory-sync --namespace <ns>
...
```bash
claude-flow hook session-start --session-id <id>
...
```bash
claude-flow hook session-restore --session-id <id>
...
Examples:
  claude-flow hook session-restore --session-id "swarm-20241019"
  claude-flow hook session-restore -s "feature-auth" --restore-memory
```
...
```bash
claude-flow hook session-end [options]
...
Examples:
  claude-flow hook session-end --session-id "dev-session-2024"
  claude-flow hook session-end -s "feature-auth" --export-metrics --generate-summary
  claude-flow hook session-end -s "quick-fix" --cleanup-temp
```
...
```bash
claude-flow hook notify --message <msg>
...
Examples:
  claude-flow hook notify -m "Task completed" --level info
  claude-flow hook notify -m "Critical error" --level error --broadcast
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "claude-flow hook pre-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/current'"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "claude-flow hook pre-bash --command '${tool.params.command}'"
        }]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "claude-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'swarm/editor/complete' --auto-format --train-patterns"
        }]
      },
      {
        "matcher": "^Bash$",
        "hooks": [{
          "type": "command",
          "command": "claude-flow hook post-bash --command '${tool.params.command}' --update-metrics"
        }]
      }
    ]
  }
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook pre-edit --file '${tool.params.file_path}' --auto-assign-agent --validate-syntax",
            "timeout": 3000,
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook pre-task --description '${tool.params.task}' --auto-spawn-agents --load-memory",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook pre-search --query '${tool.params.pattern}' --check-cache"
          }
        ]
      }
    ],
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook post-edit --file '${tool.params.file_path}' --memory-key 'edits/${tool.params.file_path}' --auto-format --train-patterns",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook post-task --task-id '${result.task_id}' --analyze-performance --store-decisions --export-learnings",
            "async": true
          }
        ]
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook post-search --query '${tool.params.pattern}' --cache-results --train-patterns"
          }
        ]
      }
    ],
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook session-start --session-id '${session.id}' --load-context"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook session-end --session-id '${session.id}' --export-metrics --generate-summary --cleanup-temp"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "claude-flow hook check-protected --file '${tool.params.file_path}'"
          }
        ]
      }
    ]
...
```javascript
// Hook command
claude-flow hook pre-task --description "Build REST API"
...
```javascript
// Hook command
claude-flow hook post-edit --file "api/auth.js"
...
```javascript
// Hook command
claude-flow hook session-end --session-id "dev-2024"
...
for FILE in $FILES; do
  ...
  claude-flow hook pre-edit --file "$FILE" --validate-syntax
  ...
  claude-flow hook post-edit --file "$FILE" --auto-format
done
...
# Run quality checks
claude-flow hook session-end \
  --generate-report \
  --export-metrics

# Verify quality thresholds
TRUTH_SCORE=$(claude-flow metrics score --format json | jq -r '.truth_score')
...
claude-flow hook session-restore \
...
claude-flow hook session-start --session-id "fullstack-feature"
...
# Backend work
claude-flow hook pre-edit --file "api/profile.js"
# ... implement backend ...
claude-flow hook post-edit \
  --file "api/profile.js" \
...
claude-flow hook pre-edit --file "components/Profile.jsx"
# ... implement frontend ...
claude-flow hook post-edit \
...
claude-flow hook session-start --session-id "debug-memory-leak"
...
claude-flow hook pre-search --query "EventEmitter"
# ... search executes ...
claude-flow hook post-search \
  --query "EventEmitter" \
  --cache-results
...
claude-flow hook pre-task --description "Analyze code complexity"
# ... analysis ...
claude-flow hook post-task \
  --task-id "analysis" \
  --store-decisions
...
for file in src/**/*.js; do
  claude-flow hook pre-edit --file "$file" --backup-file
  # ... refactor ...
  claude-flow hook post-edit \
    --file "$file" \
...
claude-flow hook pre-edit --file "test.js" --debug

# Check hook execution logs
cat .claude-flow/logs/hooks-$(date +%Y-%m-%d).log

# Validate configuration
claude-flow hook validate-config
```
...
- `claude-flow init --hooks` - Initialize hooks system
- `claude-flow hook --list` - List available hooks
- `claude-flow hook --test <hook>` - Test specific hook
- `claude-flow memory usage` - Manage memory
- `claude-flow agent spawn` - Spawn agents
